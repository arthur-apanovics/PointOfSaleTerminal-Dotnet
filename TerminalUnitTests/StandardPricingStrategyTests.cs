...
namespace TerminalUnitTests
{
    public class StandardPricingStrategyTests
    {
        ...
        {
            Action sut = () => { new StandardPricingBuilder().Build(); };
        ...
        public void GetPrice_WithSingleProduct_ReturnsPriceForProductCode()
        {
            ...
            const string code = "W";
            var sut = new StandardPricingBuilder().WithPricing(new Product(code, expected)).Build();
        ...
        [InlineData("", 0)]
        public void CalculateTotal_MixedCodes_CalculatesTotal(string sequence, decimal expected)
        {
        ...
        [InlineData("A", 0, 0)]
        public void CalculateTotal_CodeQuantities_CalculatesTotal(string code, int quantity, decimal expected)
        {
            var pricing = new List<Product>
            {
                new("A", 1.25m),
        ...
        public void CalculateTotal_InvalidQuantity_ThrowsException()
        {
            Action sut = () =>
            {
                var strategy = new StandardPricingBuilder().WithValidPricing().Build();
